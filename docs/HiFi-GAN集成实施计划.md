...
### 4.6.5 补记：分块推理（长音频卡在 10%）

**现象**：处理整首歌时，前端长时间停在 10%（"解码完成，提取 mel"）看似卡死。

**根因**：`enhance` 原先是**整段一次性前向**（`audio_to_mel(整段)` → `mel_to_audio(整段)`），
而进度只在**一个声道全部跑完**后才推进一次。长音频因此有数分钟没有任何进度事件。
附带问题：中间张量随时长线性增长，长音频内存峰值可达数 GB。

对照 `backend.rs:71` 的注释可知，本项目的约定是「**Python 侧自行切块**」——Rust 只按后端
下发 `chunk_seconds`（HiFi-GAN 默认 30 s、overlap 0），FlashSR 的 Python 也是这么做的；
HiFi-GAN 的 pipeline 漏掉了这一环。

**修复**：`pipeline.enhance` 改为按 `chunk_seconds` 切块（缺省 `DEFAULT_CHUNK_SECONDS=30`），
逐块 mel → 前向 → 拼接，并**逐块回报进度**；每块两侧多取 `CONTEXT_PAD_SECONDS=0.5` 作为
上下文、生成后按比例裁掉，以消除卷积感受野在块边界的伪影。取消检查也下沉到每块，
长任务可及时响应取消。

**验证**（25 s 立体声 + `chunk_seconds=5`，强制 5 块/声道）：推理进度事件 **10 个**
（5 块 × 2 声道），百分比 `18→26→…→90` 单调递增，`processed_seconds` `2.5→25.0` 平滑推进；
输出 `48000 Hz / 2 ch / 24.96 s`。短音频（单块）路径无回归：3 s 输入仍得
`2.9954 s / 48000 Hz / 2 ch`，与改动前一致。
