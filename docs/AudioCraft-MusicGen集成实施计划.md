...
> 状态：规划中（**G1 授权已决策通过**；G2 硬件 / G3 环境仍待阶段 0 实测）
> **G1 决策（2026-09-09）**：本产品**不做商业化，仅学习 / 研究使用**
> → MusicGen 权重的 **CC-BY-NC 4.0** 满足要求（约束见 §1.5）
> 编制日期：2026-09-09
...
| # | 门禁 | 依据 | 不通过的后果 |
|---|---|---|---|
| **G1 · 授权** | AudioCraft **代码 MIT**，但**模型权重 CC-BY-NC 4.0（非商业）** | 官方 README License 段 | ✅ **已决策通过（2026-09-09）**：**不做商业化，仅学习 / 研究使用** → CC-BY-NC 4.0 **满足要求**。须遵守：① 不得用于任何商业用途或商业分发；② UI 与文档标注模型许可与来源；③ 保留 CC-BY 署名，不去除权重自带许可声明 |
...
> **当前状态**：**G1 已通过**（非商业 / 学习研究用途）。
> 剩余 **G2（硬件实测：CPU/GPU 生成耗时）** 与 **G3（Python 3.9 + torch 2.1.0 环境可安装性）**
> 需在阶段 0 通过实测与安装验证确认，两者均通过后方可进入阶段 1。

#### 阶段 0 预检结果（2026-09-09 只读实测）

| 门禁 | 结果 | 实测依据 |
|---|---|---|
| G1 授权 | ✅ **通过** | 确认：不做商业化，仅学习 / 研究使用 |
| G2 硬件 | ⚠️ **当前机器未通过** | `nvidia-smi` 命令不可用 → **无 NVIDIA GPU / 无 CUDA**，本机为**纯 CPU** 环境 |
| G3 环境 | ⚠️ **当前机器未通过** | `py -0` 仅列出 **Python 3.10 (64-bit)**；AudioCraft 要求 **Python 3.9** → 需另行安装 3.9 |

**影响与对策**

- **G2（纯 CPU）**：MusicGen 为自回归生成，CPU 下极慢。可行路径是
  **只用 `musicgen-small`(300M) + 短时长（≤10 s）**，并在 UI 明确标注"建议 GPU"。
  ⚠️ **阶段 0 必须先实测 10 s 生成的真实耗时**再决定是否继续；
  若慢到不可接受，本模块应降级为"**实验性功能 / 需 GPU 环境**"，而非默认能力。
  （`musicgen-medium` 1.5B 在纯 CPU 上基本不可行，不建议纳入首版。）
- **G3（Python 3.9）**：需安装 Python 3.9（与现有 3.10 并存），据此创建 `.venv-musicgen`；
  并确认 `find_python_musicgen()` 能稳定定位 **3.9** 解释器
  （不可复用 `find_python*` 现有解析，否则会误选 3.10 导致 audiocraft 安装/运行失败）。
...
| # | 现象（预期风险） | 根因 | 缓解 / 修复 |
|---|---|---|---|
| **R1** | 授权不合规 | MusicGen 权重 **CC-BY-NC 4.0（非商业）** | ✅ **已由 G1 决策缓解**（2026-09-09 确认"不做商业化，仅学习/研究使用"）。仍须落实：UI 与文档标注许可与来源、保留 CC-BY 署名、不去除权重许可声明。⚠️ **若将来转向商用，必须重新评估（届时需更换为可商用模型）** |
