...
用途：在没有 HiFi-GAN 权重、也没有 GPU 的机器上验证 JSONL 协议的四种事件
（ready / progress / result / error）与取消路径，以及后续 Rust 侧集成。
与 `python/audio_ai_flashsr/fake_worker.py` 的定位相同，仅供开发期使用。

    python python/audio_ai_hifigan/fake_worker.py --mode success
"""
...
WORKER_VERSION = "fake-hifigan-0.1.0"
MODEL_ID = "hifigan-48k"
...
def process_request(request: dict[str, Any], mode: str, cancel_event: threading.Event) -> None:
    ...
    if mode == "crash":
        print("fake hifigan worker process crash", file=sys.stderr, flush=True)
        os._exit(1)
    if mode == "error":
        emit_error(request_id, WorkerFailure("INFERENCE_FAILED", "fake hifigan inference failure"))
        return
    ...
    for index in range(steps + 1):
        if cancel_event.is_set():
            emit_error(request_id, WorkerFailure("CANCELLED", "fake hifigan cancelled"))
            return
        emit(
            {
    ...
    if output_path:
        ...
        Path(output_path).write_bytes(b"fake-hifigan-output\n")
    ...
    if mode == "success-bad-exit":
        ...
        # 此时 Rust 侧不得判为失败。
        print(
            "fake hifigan worker exits non-zero after a successful result",
            file=sys.stderr,
            flush=True,
        )
        os._exit(1)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="HiFi-GAN protocol fixture worker")
    ...
    for line in sys.stdin:
        ...
        if command == "process":
            if active_thread and active_thread.is_alive():
                emit_error(
                    request_id, WorkerFailure("BUSY", "fake hifigan worker is already processing")
                )
                continue
            cancel_event.clear()
