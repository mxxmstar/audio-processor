"""HiFi-GAN 音频声码器 Worker：JSONL 协议入口与进程生命周期。

本模块顶层**只导入标准库**。Rust 侧的 ready 握手超时为 3 秒
（`src-tauri/src/audio_quality/ai_worker.rs` 的 `READY_TIMEOUT`），而
`numpy` + `torch` + vendor 的导入实测约 6 秒，因此重量级依赖必须延迟到
收到 `process` 命令之后再加载。详见实施计划风险 R12（与 FlashSR 同）。
...
WORKER_VERSION = "python-hifigan-0.1.0"
...
def _load_pipeline() -> Any:
    ...
    这里会拉起 numpy（约 0.9 s）、torch（约 5.7 s）与 vendor 上游模块，
            ...
            try:
                ...
                emit_error(
                    ...
                    WorkerFailure(
                        ...
                        f"cannot load HiFi-GAN runtime: {type(error).__name__}: {error}",
                    ),
                )
                continue
            # 必须在主线程预热上游导入：vendor 模块经 joblib/loky 拉起
            # multiprocessing 的 resource tracker，若在推理线程内导入而主线程
            # 阻塞于 stdin 读取，会死锁（stdin 为管道时必现，见 FlashSR 计划）。
            try:
