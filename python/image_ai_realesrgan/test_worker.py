"""Real-ESRGAN Worker 的清单解析与协议原语测试。

不需要权重文件，也不需要 torch —— 只依赖标准库。
运行：python python/image_ai_realesrgan/test_worker.py
"""
...
def _write_weights(model_dir: Path, contents: dict[str, bytes]) -> dict[str, str]:
    ...
    target = model_dir / "cache" / "realesrgan"
...
def _manifest(model_dir: Path, digests: dict[str, str], **overrides: object) -> None:
    entry = {
        ...
        "size_bytes": len(FILES[WEIGHT_NAME[0]]),
        "sha256": digests.get(WEIGHT_NAME[0], "0" * 64),
        "source": "https://example.invalid/RealESRGAN_x4plus.pth",
    }
...
FILES = {
    WEIGHT_NAME[0]: b"realesrgan-x4plus-bytes",
}


class ManifestTests(unittest.TestCase):
    def test_reads_single_file_entry(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            entry = manifest["realesrgan-x4plus"]
            self.assertEqual(entry["backend"], "realesrgan")
            self.assertEqual(entry["version"], "test-version")
            self.assertEqual(entry["scale"], 4)
            self.assertEqual(
                [artifact.name for artifact in entry["artifacts"]], list(WEIGHT_NAME)
            )
    ...
    def test_skips_unsupported_backend_entries(self) -> None:
        ...
        # （否则引入 realesrgan 会破坏既有 FlashSR / AudioSR / HiFi-GAN 功能）。
        with tempfile.TemporaryDirectory() as directory:
            model_dir = Path(directory)
            _write_weights(model_dir, FILES)
            realesrgan_entry = {
            ...
            model_dir.joinpath("manifest.json").write_text(
                json.dumps(
                    {"models": [realesrgan_entry, foreign_entry]},
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            )
            ...
            assert manifest is not None
            self.assertIn("realesrgan-x4plus", manifest)
    ...
    def test_rejects_path_escape(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            text = model_dir.joinpath("manifest.json").read_text(encoding="utf-8")
            model_dir.joinpath("manifest.json").write_text(
                text.replace("cache/realesrgan/RealESRGAN_x4plus.pth", "../escaped.bin"),
                encoding="utf-8",
            )
...
class FindModelTests(unittest.TestCase):
    def test_find_model_verifies_size_and_hash(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            spec = worker.find_model("realesrgan-x4plus", model_dir)
            self.assertEqual(spec.model_id, "realesrgan-x4plus")
            self.assertEqual(spec.version, "test-version")
            self.assertEqual(spec.scale, 4)
            self.assertEqual([name for name, _ in spec.artifacts], list(WEIGHT_NAME))

    def test_find_model_detects_hash_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            model_dir = Path(directory)
            digests = _write_weights(model_dir, FILES)
            digests[WEIGHT_NAME[0]] = "0" * 64
            _manifest(model_dir, digests)
            with self.assertRaises(worker.WorkerFailure) as raised:
                worker.find_model("realesrgan-x4plus", model_dir)
    ...
    def test_find_model_detects_size_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            text = model_dir.joinpath("manifest.json").read_text(encoding="utf-8")
            text = text.replace(
                f'"size_bytes": {len(FILES[WEIGHT_NAME[0]])}',
                f'"size_bytes": {len(FILES[WEIGHT_NAME[0]]) + 1}',
            )
    ...
    def test_find_model_requires_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(worker.WorkerFailure) as raised:
                worker.find_model("realesrgan-x4plus", Path(directory))
            self.assertEqual(raised.exception.code, "MODEL_NOT_FOUND")
...
class AvailableModelsTests(unittest.TestCase):
    def test_reports_missing_weight_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            models, errors = worker.available_models(model_dir)
            self.assertEqual(models, [])
            self.assertEqual(errors, ["realesrgan-x4plus: MODEL_NOT_FOUND"])
    ...
    def test_valid_weights_are_listed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            _manifest(model_dir, digests)
            models, errors = worker.available_models(model_dir)
            self.assertEqual(models, ["realesrgan-x4plus"])
...
class LazyImportTests(unittest.TestCase):
    """R12：ready 握手之前绝不能导入 torch / numpy / basicsr / realesrgan。

    Rust 侧的 READY_TIMEOUT 只有 3 秒，而 torch 导入实测约 5+ 秒。
    ...
    def _assert_clean_import(self) -> None:
        code = (
            ...
            "import worker;"
            "leaked = [m for m in ('torch', 'numpy', 'basicsr', 'realesrgan') if m in sys.modules];"
