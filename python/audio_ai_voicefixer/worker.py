"""VoiceFixer 语音修复 Worker：JSONL 协议入口与进程生命周期。

本模块顶层**只导入标准库**。Rust 侧的 ready 握手超时为 3 秒
（`src-tauri/src/audio_quality/ai_worker.rs` 的 `READY_TIMEOUT`），而
`numpy` + `torch` + `voicefixer` 的导入实测约 7 秒，因此重量级依赖必须延迟
到收到 `process` 命令之后再加载（与 FlashSR / HiFi-GAN 同源的风险 R12）。
...
WORKER_VERSION = "python-voicefixer-0.1.0"
MODULE_ROOT = Path(__file__).resolve().parent

#: VoiceFixer 后端在 manifest 中的 backend 标识与模型 id。
SUPPORTED_BACKENDS = frozenset({"voicefixer"})
VOICEFIXER_MODEL_ID = "voicefixer"

#: 清单里两个权重文件的 `name`（顺序即 `ordered_weights` 的取值顺序）。
WEIGHT_NAMES = ("analysis", "vocoder")

#: VoiceFixer 原生输出采样率（§3.5：不要套用 FlashSR 的 48 kHz 校验）。
NATIVE_SAMPLE_RATE = 44100

#: 官方定义的修复模式；`0` 为默认（见实施计划 §1.3）。
...
class ModelSpec(NamedTuple):
    ...
    `artifacts` 按清单中的声明顺序保存 `(名称, 绝对路径)`。VoiceFixer 需要
    两个权重：`analysis`（分析模块 `vf.ckpt`）与 `vocoder`（44.1 kHz 声码器），
    取值时按名称显式挑选（见 `ordered_weights`）。

    `native_sample_rate` 固定为 44100（VoiceFixer 原生），与 FlashSR / HiFi-GAN
    的 48000 不同，`pipeline.enhance` 据此写回结果而**不做** 48 kHz 重采样。
    """
...
def parse_mode(request: dict[str, Any]) -> int:
    """解析请求里的 `mode`（0/1/2），缺省为 0，非法值报错。

    放在 `worker.py`（纯标准库）里，`test_worker.py` 无需 torch 即可覆盖。
    """
    raw = request.get("mode", DEFAULT_MODE)
    ...
    try:
        ...
        raise WorkerFailure(
            "UNSUPPORTED_MODE", f"mode must be one of {list(SUPPORTED_MODES)}, got {raw!r}"
        ) from error
...
def read_manifest(model_dir: Path) -> dict[str, dict[str, Any]] | None:
    ...
    try:
        ...
        for entry in entries:
            ...
            # 共享 manifest 含其它后端的条目（如 flashsr / audiosr / hifigan）；
            ...
            result[model_id] = {
                ...
                "sample_rate": int(entry["sample_rate"]) if entry.get("sample_rate") else 0,
...
def find_model(model_id: str, model_dir: Path) -> ModelSpec:
    ...
    return ModelSpec(
        ...
        native_sample_rate=int(entry.get("sample_rate") or NATIVE_SAMPLE_RATE),
        ...
        try:
            for artifact in entry["artifacts"]:
                ...
                if candidate.stat().st_size > STARTUP_HASH_MAX_BYTES:
                    # 数百 MB 的 SHA-256 会超出 Rust 侧的 ready 超时；同样的
...
def _load_pipeline() -> Any:
    ...
    这里会拉起 numpy、torch 与 voicefixer，合计约 7 秒，远超 Rust 侧 3 秒的
...
def main() -> int:
    parser = argparse.ArgumentParser(description="VoiceFixer speech restoration worker")
            ...
            try:
                ...
                emit_error(
                    ...
                    WorkerFailure(
                        ...
                        f"cannot load VoiceFixer runtime: {type(error).__name__}: {error}",
                    ),
                )
                continue
            # 必须在主线程完成上游导入：torch / voicefixer 会拉起线程池与
            # 动态库，若在推理线程内首次导入而主线程阻塞于 stdin 读取，
            # 存在死锁风险（stdin 为管道时，见 FlashSR / HiFi-GAN 计划）。
            # VoiceFixer 的缓存目录重定向也必须在这里完成（导入期即求值）。
            try:
                pipeline.warm_up_imports(model_dir)
            except Exception as error:  # noqa: BLE001
                ...
                emit_error(
                    ...
                    WorkerFailure(
                        "MODEL_RUNTIME_NOT_FOUND",
                        f"无法导入 VoiceFixer: {type(error).__name__}: {error}",
                    ),
                )
                continue
            cancel_event.clear()
