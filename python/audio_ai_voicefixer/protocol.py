...
（与 `python/audio_ai_hifigan/protocol.py` 完全一致，按目录各自持有一份，
保持各 Worker 独立成进程、可单独分发。）
"""
...
def _force_utf8_stdio() -> None:
    ...
    编码（如 cp936/GBK）解码。当路径含非 ASCII 字符（日文、罕见汉字等）时
    解码会失败或产生乱码，导致 Worker 判定「输入音频不存在」。
...
def resolve_binary(name: str, env_name: str) -> str:
    """定位 ffmpeg / ffprobe：环境变量优先，其次仓库 bin/，最后 PATH。"""
